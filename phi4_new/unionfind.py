...
class UnionFind(object):
    ...
    def __init__(self, elements=None):
        ...
        self.comps = {} # dict mapping roots -> element index belong to that component
...
#try to modify to return list     
    ...
    def union(self, x, y):
        ...
        if self._siz[xroot] < self._siz[yroot]:
            ...
            self.comps[yroot].extend(self.comps[xroot]) 
            self.comps.pop(xroot)  #it could be optional to pop these
        else:
            self._par[yroot] = xroot
            self._siz[xroot] += self._siz[yroot]
            self.comps[xroot].extend(self.comps[yroot]) 
            self.comps.pop(yroot)
        self.n_comps -= 1

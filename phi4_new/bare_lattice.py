...
class phi_4:
    ...
    def __init__(self, dim, N, kappa, lambda_0):
        ...
        self.checkerboard = np.indices(self.lattice_shape).sum(axis=0) % 2
        ...
        self.nbr = np.array([[(i // self.N) * self.N + (i + 1) % self.N, (i + self.N) % self.N**2,
                                  (i // self.N) * self.N + (i - 1) % self.N, (i - self.N) % self.N**2]
                                         for i in range(self.N**2)])


    def sum_neighbor(self, lattice):
        neighbor_sum = np.zeroes(self.lattice.shape)
        for dim in range(self.dim):
            neighbor_sum += np.roll(lattice, 1, axis=i)+np.roll(lattice, -1, axis=i)
        return neighbor_sum    
